...
public class DBhelper extends SQLiteOpenHelper {
    ...
    public ArrayList<bill>getBillCus(int i){
        ArrayList<bill> productArrayList=new ArrayList<>();
        String sql="SELECT* FROM bill WHERE IdCus="+String.valueOf(i);
        SQLiteDatabase database=this.getWritableDatabase();
        Cursor cursor=database.rawQuery(sql,null);
        if(cursor.moveToFirst()){
            do{
                bill item=new bill();
                item.setIdBill(cursor.getInt(0));
                item.setDayBill(cursor.getString(2));
                item.setAmountBill(cursor.getInt(3));
